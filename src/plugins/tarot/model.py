...
class ElementModel(TarotBaseModel):
    """元素"""
    id: int
    orig_name: str  # 原始名称
    name: str  # 名称
...
class ConstellationModel(TarotBaseModel):
    """星与星座"""
    id: int
    orig_name: str  # 原始名称
    name: str  # 名称
...
class TarotCardModel(TarotBaseModel):
    """塔罗牌"""
    id: int  # 塔罗牌的序号
    index: str  # 内部名称
...
class TarotPack(TarotBaseModel):
    """塔罗牌套牌"""
    cards: list[TarotCardModel]  # 套牌内容

    @property
    def num(self) -> int:
        return len(self.cards)

    def get_card_by_id(self, id_: int) -> TarotCardModel:
        card_map = {card.id: card for card in self.cards}
        return card_map[id_]

    def get_card_by_index(self, index_: str) -> TarotCardModel:
        card_map = {card.index: card for card in self.cards}
        return card_map[index_]

    def get_card_by_name(self, name: str) -> TarotCardModel:
        card_map = {card.name: card for card in self.cards}
...
class TarotCards(Enum):
    """所有塔罗牌卡牌"""
    @classmethod
    def get_all_cards(cls) -> list[TarotCardModel]:
        """获取所有塔罗牌的列表"""
        cards = [member.value for member in cls.__members__.values() if isinstance(member.value, TarotCardModel)]
        return cards

    @classmethod
    def get_cards_by_type(cls, *type_: str) -> list[TarotCardModel]:
        """获取某个类型的塔罗牌列表"""
