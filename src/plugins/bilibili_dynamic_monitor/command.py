...
async def handle_add_subscription(
    ...
    if ensure is None:
        ...
        await interface.send_at_sender('正在更新Bilibili用户动态订阅信息, 请稍候')
    ...
    try:
        ...
        await interface.send_at_sender('获取用户信息失败, 可能是网络原因或没有这个用户, 请稍后再试')
        return
...
async def handle_del_subscription(
    ...
    try:
        ...
        await interface.send_at_sender('获取已订阅用户动态列表失败, 请稍后再试或联系管理员处理')
        return
...
async def handle_list_subscription(interface: Annotated[OmegaInterface, Depends(OmegaInterface())]) -> None:
    ...
    try:
        exist_sub = await query_entity_subscribed_dynamic_sub_source(interface=interface)
        exist_text = '\n'.join(f'{sub_id}: {user_nickname}' for sub_id, user_nickname in exist_sub.items())
        await interface.send_at_sender(f'当前已订阅的Bilibili用户动态:\n\n{exist_text if exist_text else "无"}')
    except Exception as e:
        logger.error(f'获取{interface.entity}已订阅用户动态列表失败, {e!r}')
        await interface.send_at_sender('获取已订阅用户动态列表失败, 请稍后再试或联系管理员处理')
