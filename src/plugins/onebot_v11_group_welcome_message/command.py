...
from src.service import OmegaInterface, OmegaMessage, enable_processor_state
...
async def handle_set_welcome_message(
    ...
    try:
        parsed_message = cast(OmegaMessage, interface.get_msg_extractor(bot=bot)(message=message).message)
        ...
        logger.success(f'已为 {interface.entity} 设置了自定义欢迎消息: {message}')
...
async def handle_send_welcome_message(
    ...
    try:
        ...
        if welcome_message_setting.available == 1:
            ...
            send_message = OmegaInterface().get_msg_builder()(send_message).message
            await matcher.send(message=send_message, at_sender=True)
    except Exception as e:
        logger.warning(f'{interface.entity} 未配置自定义欢迎消息, 或发送消息失败, {e!r}')
