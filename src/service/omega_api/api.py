...
class _RouteRegisterMixin(abc.ABC):
    """提供 register_{get,post,put,delete}_route 路由注册方法的 Mixin 类"""
    ...
    @abc.abstractmethod
    def _add_route(self, method: str, path: str, func: Callable[..., Coroutine]) -> None:
        """将路由注册到目标应用/路由器"""
        raise NotImplementedError

    @abc.abstractmethod
    def _route_display_url(self, path: str) -> str:
        """日志中展示的路由 URL"""
        raise NotImplementedError
    ...
    def _register_route(self, method: str, path: str):
        ...
        :param path: 请求路径
        """
        path = f'/{self._normalize_mount_str(path)}'
        ...
        def decorator[**P, T1, T2, R](func: Callable[P, Coroutine[T1, T2, R]]) -> Callable[P, Coroutine[T1, T2, R]]:
            ...
            color = MethodLogColor[method]
            logger.opt(colors=True).info(
                f'{self.color_log_prefix} registered: (<{color}>{method}</{color}>) '
                f'<b><u>{self._route_display_url(path)}</u></b>'
            )
            return func
...
class OmegaAPIRouter(_RouteRegisterMixin, APIRouter):
    ...
    def _add_route(self, method: str, path: str, func: Callable[..., Coroutine]) -> None:
        self.api_route(path, methods=[method])(func)

    def _route_display_url(self, path: str) -> str:
        return f'{self.prefix}{path}'


class OmegaAPI(_RouteRegisterMixin):
    """Omega API 应用创建与路由注册"""
    ...
    def __init__(
        ...
        self.__used_signatures: dict[str, float] = {}
    ...
    def _is_replayed_signature(self, signature: str) -> bool:
        """检查签名是否已被使用过, 未使用则记录该签名(防重放)

        缓存达到容量阈值时会先清理过期条目, 清理后仍超限则直接拒绝新签名, 保证缓存有界
        """
        now = time.monotonic()
        if len(self.__used_signatures) >= _USED_SIGNATURES_MAX_SIZE:
            expired = [x for x, expire_at in self.__used_signatures.items() if expire_at <= now]
            for x in expired:
                del self.__used_signatures[x]
            if len(self.__used_signatures) >= _USED_SIGNATURES_MAX_SIZE:
                # 清理后仍超限: 拒绝新签名, 防止缓存被撑爆(OOM), TTL 到期后自动恢复
                logger.opt(colors=True).warning(
                    f'<lc>Omega API</lc> | used signatures cache is full (size={len(self.__used_signatures)}), '
                    f'reject new signature'
                )
                return True
        expire_at = self.__used_signatures.get(signature)
        if expire_at is not None and expire_at > now:
            return True
        self.__used_signatures[signature] = now + _USED_SIGNATURES_TTL
        return False
    ...
    def _normalize_sign_params(params: Mapping[str, str]) -> str:
        ...
        if hasattr(params, 'multi_items'):
            pairs = list(params.multi_items())  # type: ignore
        else:
    ...
    def sign_params_hmac(
        ...
        *,
        ...
        params_json = cls._normalize_sign_params(params)
        body_hash = sha256(body).hexdigest()
        sign_message = f'{app_name}.{method.upper()}.{path}.{timestamp}.{params_json}.{body_hash}'
        app_key = cls._derive_app_key(key, app_name)
    ...
    @staticmethod
    async def _read_request_body(request: Request, max_size: int) -> bytes | None:
    ...
    def _log_rejected(self, request: Request, reason: str) -> None:
        """记录校验失败的审计日志"""
    ...
    def _init_sub_app(self) -> FastAPI:
        ...
        async def token_verify_middleware(request: Request, call_next):
            ...
            if request_app is None or request_app != self._app_name:
                self._log_rejected(request, 'Invalid Request App')
            ...
            if request_timestamp is None:
                self._log_rejected(request, 'Timestamp Not Provided')
            ...
            if not request_timestamp.isdecimal() or len(request_timestamp) > 16:
                self._log_rejected(request, 'Invalid Timestamp')
            ...
            if token is None:
                self._log_rejected(request, 'Token Not Provided')
            ...
            if content_length is not None and (
                not content_length.isdecimal() or int(content_length) > _REQUEST_BODY_MAX_SIZE
            ):
                self._log_rejected(request, 'Payload Too Large')
                return JSONResponse({'error': True, 'message': 'Payload Too Large'}, status_code=413)

            body = await self._read_request_body(request, _REQUEST_BODY_MAX_SIZE)
            if body is None:
                self._log_rejected(request, 'Payload Too Large')
                return JSONResponse({'error': True, 'message': 'Payload Too Large'}, status_code=413)
            ...
            if not await self.async_verify_params_hmac(
                ...
                self._log_rejected(request, 'Invalid Token')
                return JSONResponse({'error': True, 'message': 'Invalid Token'}, status_code=403)

            # 防重放校验, 已使用过的合法签名在缓存有效期内将被拒绝
            if self._is_replayed_signature(token):
                self._log_rejected(request, 'Replayed Token')
    ...
    def mount_router[T: APIRouter](self, api_router: T, prefix: str = '', **kwargs) -> T:
        """挂载 APIRouter"""
        prefix = f'/{self._normalize_mount_str(prefix)}' if prefix else prefix
    ...
    def mount_static_path(
        ...
        prefix = f'/{self._normalize_mount_str(prefix)}' if prefix else ''
        path = f'/{self._normalize_mount_str(path)}' if path else ''
        mount_path = f'{prefix.strip()}{path.strip()}'.strip()
        if not mount_path:
            raise ValueError('path and prefix can not both be empty')
    ...
    def _add_route(self, method: str, path: str, func: Callable[..., Coroutine]) -> None:
        self._app.api_route(path, methods=[method])(func)
