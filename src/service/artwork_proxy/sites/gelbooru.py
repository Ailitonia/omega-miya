...
class BaseGelbooruArtworkProxy(BaseArtworkProxy, abc.ABC):
    ...
    async def _query(self) -> ArtworkData:
        artworks_data = await self._get_api().post_show(id_=self.i_aid)
        if not artworks_data.post:
            raise WebSourceException(f'GelbooruArtwork(id={self.i_aid}) Not Found')
        artwork_data = artworks_data.post[0]
