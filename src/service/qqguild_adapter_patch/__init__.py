...
async def send(
    ...
    if isinstance(event, DirectMessageCreateEvent):
        ...
        return await self.send_to_dms(message=message, guild_id=event.guild_id, msg_id=event.id, **kwargs)
    else:
        return await self.send_to(message=message, channel_id=event.channel_id, msg_id=event.id, **kwargs)
