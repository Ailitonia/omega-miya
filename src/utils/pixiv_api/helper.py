...
class PixivPreviewGenerator(PixivApiBase):
    ...
    async def generate_user_searching_result_image(
        ...
        tasks = [cls._generate_user_searching_result_card(user=user, width=width) for user in users]
    ...
    async def _request_artwork_preview_body(
        ...
        _artwork_model = await artwork.query_artwork()
        _artwork_thumb = await artwork.get_page_bytes(url_type='small')
