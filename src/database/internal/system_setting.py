...
class SystemSettingDAL(BaseDataAccessLayerModel[SystemSettingOrm, SystemSetting]):
    ...
    async def upsert(self, setting_name: str, setting_value: str, info: Optional[str] = None) -> None:
        new_obj = SystemSettingOrm(
            setting_name=setting_name, setting_value=setting_value, info=info, updated_at=datetime.now()
        )
    ...
    async def update(
        ...
        stmt = update(SystemSettingOrm).where(SystemSettingOrm.setting_name == setting_name)
    ...
    async def delete(self, setting_name: str) -> None:
        stmt = delete(SystemSettingOrm).where(SystemSettingOrm.setting_name == setting_name)
        stmt.execution_options(synchronize_session="fetch")
        await self.db_session.execute(stmt)
