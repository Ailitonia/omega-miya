...
from .exception import CalculateException


class BaseCalculator(object):
    # 限制单步运算算数上限, 避免进行大数运算
    __power_limit: float = 65536
    __multi_limit: float = 4294967296
    __add_limit: float = 4294967296

    def __init__(self, expression: str):
        self.__raw_expression: str = expression
        # 移除空白字符
        __expression = re.sub(r'\s', '', expression)
        # 替换中文符号
        __expression = re.sub(r'（', '(', __expression)
        __expression = re.sub(r'）', ')', __expression)
        # 替换运算符
        __expression = re.sub(r'[xX×]', '*', __expression)
        __expression = re.sub(r'[÷]', '/', __expression)
        self.__expression: str = __expression
        ...
        if '^' in calculate_sequence:
            ...
            for _index, _obj in reversed(list(enumerate(calculate_sequence))):
                ...
                if _obj == '^':
                    ...
                    if num_front >= cls.__power_limit or num_back >= cls.__power_limit:
            ...
            for _index, _obj in enumerate(calculate_sequence):
                ...
                if _obj == '*':
                    ...
                    calculate_sequence.pop(_index)
                    num_front = float(calculate_sequence.pop(_index - 1))
                    if num_front >= cls.__multi_limit or num_back >= cls.__multi_limit:
                        raise CalculateException('单步运算算数大小超过限制上限', f'{num_front}, {num_back}')
                    result = num_front * num_back
                    calculate_sequence.insert(_index - 1, result)
                    return calculate_sequence
            ...
            for _index, _obj in enumerate(calculate_sequence):
                ...
                if _obj == '+':
                    ...
                    calculate_sequence.pop(_index)
                    num_front = float(calculate_sequence.pop(_index - 1))
                    if num_front >= cls.__add_limit or num_back >= cls.__add_limit:
                        raise CalculateException('单步运算算数大小超过限制上限', f'{num_front}, {num_back}')
                    result = num_front + num_back
                    calculate_sequence.insert(_index - 1, result)
                    return calculate_sequence
    ...
    async def aio_std_calculate(self) -> float:
        loop = asyncio.get_running_loop()
        result = await loop.run_in_executor(None, self.std_calculate)
        return result
