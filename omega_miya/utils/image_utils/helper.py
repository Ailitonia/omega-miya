...
async def generate_thumbs_preview_image(
        ...
        *,
        font_path: LocalResource = image_utils_config.default_preview_font,
        header_color: tuple[int, int, int] = (255, 255, 255),
        hold_ratio: bool = False,
        num_of_line: int = 6,
        limit: int = 1000,
        output_folder: TmpResource = image_utils_config.default_preview_img_folder
) -> TmpResource:
    """生成多个带说明的缩略图的预览图
    ...
    def _handle_preview_image() -> bytes:
        ...
        # 标题自动换行
        _title = TextUtils(text=preview_name).split_multiline(
            width=int(_preview_w * 0.85), font=_font_title).text
        # 计算标题尺寸
