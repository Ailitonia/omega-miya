...
async def handle_sticker(bot: Bot, event: Event, state: dict):
    # 定义模板名称、类型, 处理模板正则
    sticker_temp = {
...
async def handle_img(bot: Bot, event: Event, state: dict):
...
async def handle_sticker_text(bot: Bot, event: Event, state: dict):
    ...
    try:
        ...
        logger.error(f'Group: {event.group_id}, 用户: {event.user_id} 制作表情时发生了错误: {repr(e)}')
