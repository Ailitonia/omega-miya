...
async def get_http_cat(http_code: int) -> Result.TextResult:
    ...
    if os.path.exists(file_path):
        return Result.TextResult(error=False, info='Success, file exists', result=f'file:///{file_path}')
    ...
    if result.success():
        return Result.TextResult(error=False, info='Success', result=f'file:///{result.result}')
    else:
